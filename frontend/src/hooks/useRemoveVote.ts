...
/**
 * Custom hook for removing votes from Kanban cards with optimistic updates
 * Uses TanStack Query for mutation management and cache updates
 * 
 * @param projectSlug - URL-friendly project identifier
 * @returns Mutation object with remove vote function and loading/error states
 */
export function useRemoveVote(projectSlug: string) {
  const queryClient = useQueryClient();
  
  return useMutation<VoteResponse, Error, number, { previousBoard: KanbanBoard | undefined }>({
    mutationFn: async (cardId: number) => {
      try {
        ...
        if (error.response?.status === 404) {
          throw new Error('You have not voted on this card.');
        }
